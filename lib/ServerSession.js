...
ServerSession.prototype.summary = function(){
	console.log('===========================');
	console.log('===========================');
	for(var id in this.clientSessions){
		console.log(id);
	}
	console.log('');
}
...
ServerSession.prototype.destroySession = function(socket){
	if(this.clientSessions.hasOwnProperty(socket.id)){
		...
		// If needed, log user out 
		this.clientSessions[socket.id].logOut();

		// Remove client from client room
		var location = this.clientSessions[socket.id].location;
		this.clientSessions[socket.id].location = null;
		gRooms.removeSocket(location, socket);
